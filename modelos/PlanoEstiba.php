...
Class Plano
{
	...
	public function insertarDetalle($idPlano_estiba,$bodega,$seccion_bodega,$idProducto,$peso, $no_bultos)
	{
		$sql="INSERT INTO detalle_plano(idPlano_Estiba,bodega,seccion_bodega,idProducto,peso,no_bultos,estado)
		VALUES ('$idPlano_estiba','$bodega','$seccion_bodega','$idProducto','$peso','$no_bultos','I')";
	       	//return ejecutarConsulta($sql);
		return ejecutarConsulta($sql);
	}

	public function editarDetalle($idDetalle_plano,$idPlano_estiba,$bodega,$seccion_bodega,$idProducto,$peso,$no_bultos)
	{
		$sql="UPDATE detalle_plano set idPlano_Estiba = '$idPlano_estiba', bodega = '$bodega', seccion_bodega = '$seccion_bodega', idProducto = '$idProducto', peso = '$peso', no_bultos='$no_bultos' where idDetalle_Plano = '$idDetalle_plano'";
		//return ejecutarConsulta($sql);
		return ejecutarConsulta($sql);
	}
	...
	public function ListarDetalles($idPlano,$bodega)
	{
		...
		$sql="SELECT dp.idPlano_Estiba, dp.idDetalle_Plano, p.nombre as producto, dp.peso, dp.idProducto, dp.peso, dp.bodega, dp.seccion_bodega, dp.peso_bodega, dp.no_bultos, dp.estado from detalle_plano dp inner join producto p on dp.idProducto = p.idProducto where dp.idPlano_Estiba='$idPlano' and dp.bodega = '$bodega'";
		return ejecutarConsulta($sql);
	}	
	...
	public function CodigoDes($idPlano,$licencia)
	{
		$sql = " SELECT d.idDespacho, d.idAsignacion, d.codigo, da.codigo as codigo_asignacion,concat(p.nombre,' ',p.apellido) as piloto,p.licencia
				...
				where dp.idPlano_Estiba = '$idPlano' and d.estado !='A' and p.licencia = '$licencia' and d.idDespacho NOT IN(SELECT idDespacho FROM salida)";
		return ejecutarConsulta($sql);
	}	
