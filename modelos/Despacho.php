...
Class Despacho
{
	...
	public function insertar($idAsignacion,$idAlmeja,$idTolva,$codigo,$hora,$minuto,$segundo,$seccion_bodega,$fecha,$bultos_descargados,$motivo,$horat,$minutot,$segundot)
	{
echo $seccion_bodega;
		$sql="INSERT INTO despacho(idAsignacion,idAlmeja,idTolva,codigo,hora,minuto,segundo,seccion_bodega,fecha,bultos_descargados,estado)
		VALUES ('$idAsignacion','$idAlmeja','$idTolva','$codigo','$hora','$minuto','$segundo','$seccion_bodega','$fecha','$bultos_descargados','D')";
	echo $sql;	
	   $idDespacho=ejecutarConsulta_retornarID($sql);

		$codigo = $idDespacho < 10 ? 'CD-0'.$idDespacho:'CD-'.$idDespacho;
		$sqlCodigo = " UPDATE despacho SET codigo='$codigo' WHERE idDespacho ='$idDespacho'";
	...
	{
		$sql="SELECT d.idDespacho, d.idAlmeja, d.idTolva,d.idAsignacion,d.seccion_bodega, d.bultos_descargados, ea.descripcion, d.codigo, d.hora, d.minuto, d.segundo, ea.descripcion as almeja, et.descripcion as tolva, da.codigo as codigo_asignacion FROM despacho d INNER JOIN equipo ea on d.idAlmeja = ea.idEquipo inner join equipo et on d.idTolva = et.idEquipo inner join detalle_asignacion da on d.idAsignacion = da.idDetalle_asignacion where d.estado !='A'";
