...
$no_bultos=isset($_POST["no_bultos"])? limpiarCadena($_POST["no_bultos"]):"";
...
switch ($_GET["op"]){
	...
	case 'guardardetalle':
		if (empty($idDetalle_plano)){
			$rspta=$plano->insertarDetalle($idPlano_estiba,$bodega,$seccion_bodega,$idProducto,$peso,$no_bultos);
			echo $rspta ? "Bodega ha sido registrada" : "no se pudo registrar Bodega";
		}
		else {
			$rspta=$plano->editarDetalle($idDetalle_plano,$idPlano_estiba,$bodega,$seccion_bodega,$idProducto,$peso,$no_bultos);
